...
], function(Handlebars, moment, util, mvCookie) {
  'use strict';
...
Handlebars.registerHelper('not_if_or', function(v1, v2, v3, options) {
  if (!options) {
    // only 2 params were passed, and v3 is options
    options = v3;
    if (v1 || v2) {
  ...
  Handlebars.registerHelper('bopsFacetStoreName', function(facetName, facets, storeNumber) {
    var name, store;
  ...
  Handlebars.registerHelper('normalizeIfPrice', function(facetName, facetValue, brand) {
    if (facetName === 'PRICE' || facetName === undefined) {
  ...
  Handlebars.registerHelper('formatNumber', function(number, decimalPoints) {
    return parseFloat(number).toFixed(arguments.length > 2 ? decimalPoints : 2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  });
  ...
  Handlebars.registerHelper('signInUrl', function(urlParamAppend) {
    return util.getSignInUrl(urlParamAppend);
  });

  Handlebars.registerHelper('signOutUrl', function(urlParamAppend) {
    return util.getSignOutUrl(urlParamAppend);
  });
  ...
  Handlebars.registerHelper('relativeUrl', function(urlPath) {
    var relativeUrl, $url = $.url(urlPath);
  ...
  Handlebars.registerHelper('buildImagePath', function(productInfo) {
    var displayImageName,
        type,
        path;

    if (productInfo.upcPrimaryImageName){
      displayImageName = productInfo.upcPrimaryImageName;
      type = 'products';
    } else if (productInfo.swatchImageName) {
      displayImageName = productInfo.swatchImageName;
      type = 'swatches';
    } else if (productInfo.primaryImageName) {
      displayImageName = productInfo.primaryImageName;
      type = 'products';
    }

    if (displayImageName){
      var imageid = displayImageName.split('.')[0];
      if (imageid){
        var lastDigit = imageid.charAt (imageid.length - 1);
        path = App.config.paths.bag.imageRoot + '/' + type + '/' + lastDigit + '/optimized/' + imageid + '_fpx.tif?wid=198&fmt=jpeg&qlt=90';
      }
    }
  ...
  Handlebars.registerHelper('showBagItemColorSwatch', function(productInfo, options) {
    if (!productInfo.upcPrimaryImageName && productInfo.swatchImageName) {
      return options.fn(this);
    }
  ...
  Handlebars.registerHelper('buildBagItemImagePath', function(productInfo) {
    var displayImageName,
        path;

    displayImageName = productInfo.upcPrimaryImageName || productInfo.primaryImageName;

    if (displayImageName){
      var imageid = displayImageName.split('.')[0];
      if (imageid) {
        var lastDigit = imageid.charAt (imageid.length - 1);
        path = App.config.paths.bag.imageRoot + '/products/' + lastDigit + '/optimized/' + imageid + '_fpx.tif?wid=150&fmt=jpeg&qlt=90';
      }
    }
  ...
  Handlebars.registerHelper('buildPdvImagePath', function(images) {

    var displayImageName = images[Object.keys(images)[0]][0].image,
        path;

    if (displayImageName){
      var imageid = displayImageName.split('.')[0].replace('_fpx', '');
      if (imageid) {
        var lastDigit = imageid.charAt (imageid.length - 1);
        path = App.config.paths.bag.imageRoot + '/products/' + lastDigit + '/optimized/' + imageid + '.tif?wid=150&fmt=jpeg&qlt=90';
      }
    }
  ...
  Handlebars.registerHelper('buildBagItemColorSwatchImagePath', function(productInfo) {
    var displayImageName,
        path;
    ...
    if (displayImageName) {
      ...
      if (imageid) {
        var lastDigit = imageid.charAt (imageid.length - 1);
        path = App.config.paths.bag.imageRoot + '/swatches/' + lastDigit + '/optimized/' + imageid + '_fpx.tif?wid=150&hei=150&fmt=jpeg&qlt=90&fit=fit,1';
      }
    }
  ...
  Handlebars.registerHelper('restResetPasswordOn', function(options) {
    var $restResetPasswordOn = App.config.ENV_CONFIG.rest_reset_password === 'on' ? true : false;
    if ($restResetPasswordOn) {
      return options.fn(this);
    } else {
      return options.inverse(this);
    }
  ...
  Handlebars.registerHelper('makeRegistryName', function(registry) {
    if(!(registry.regstrantFName || registry.registrantLName) && !(registry.coregistrantFirstName || registry.coregistrantLastName)){
      return "Registrant's name is temporarily unavailable";
    }

    return registry.registrantFName + ' ' + registry.registrantLName + ' & ' + registry.coregistrantFirstName + ' ' + registry.coregistrantLastName;
  });

  Handlebars.registerHelper('ifDisplayRegistryHeader', function(items, itemIndex, options) {
    var thisItem = items[itemIndex];
    if (thisItem.registry && thisItem.registry.id) {

      //If this is the first item in the list,
      //or the previous item is not a registry item
      //or the previous item is a registry item, but is from a defferent registry
      //we need to display the header
      if (itemIndex === 0 || !items[itemIndex - 1].registry || (items[itemIndex - 1].registry && items[itemIndex - 1].registry.id !== thisItem.registry.id)) {
        return options.fn(this);
      }
    }
  ...
  Handlebars.registerHelper('getOfferPrice', function(promotionOffers, id) {
    var price;
    _.each(promotionOffers, function(promo){
        if(promo.giftProductId === id)
        {
          price = promo.yourPrice;
          return;
        }
      });
    return Handlebars.helpers.formatNumber(price, 2);
  });

  /**
  **Two type expiry.
  **ExpiringSoon: If any deal is expiring in 48 hours
  **NeverExpire: If any deal is having expiry more than 2 years
  **/
  Handlebars.registerHelper('checkPromotionExpiration', function(promotions, type, options) {
    var dealExpiryLimit = 2;
    if (type === 'NeverExpire'){
      dealExpiryLimit = 730;
    }

    var currentEsternTime = moment.utc().zone('-0500');
    var dealExpiryDay = promotions.expirationDate;
    var dealExpiryTime = moment(dealExpiryDay + ' 23:59', 'MM/DD/YYYY HH:mm').utc().zone('-0500');
    if (dealExpiryTime > currentEsternTime){
      //var differenceInDays = moment.utc(dealExpiryTime.diff(currentEsternTime)).format('DD');
      var differenceInDays = dealExpiryTime.diff(currentEsternTime, 'days');
      if (parseInt(differenceInDays) <= dealExpiryLimit){
        return options.fn(this);
      }
    }
  ...
  Handlebars.registerHelper('isiOS', function(options) {
    if (util.isiOS()){
      return options.fn(this);
    }else {
      return options.inverse(this);
  ...
  Handlebars.registerHelper('ifIsPWP', function(specials, options) {
    if (_.indexOf (specials, 'PWP') !== -1){
      return options.fn(this);
    }else {
      return options.inverse(this);
    }
  ...
  Handlebars.registerHelper('isOfferInWallet', function(offersInWallet, promoCode, options) {
    var isOfferInWallet = false;
    if (!_.isEmpty(offersInWallet) && !_.isEmpty(offersInWallet)){
      if (_.contains(offersInWallet, promoCode)){
        isOfferInWallet = true;
      }
    }
    if (isOfferInWallet){
      return options.fn(this);
  ...
  /**
    Decorates the bag ID with a '-' after the 4th digit
  */
  Handlebars.registerHelper('formatBagID', function(bagId) {
    if (bagId){
      bagId = bagId.toString();
      if (bagId.length > 4){
        return bagId.substr(0, 4) + '-' + bagId.substr(4);
      }
    }

  });

  var freeShipping = function(promo) {
    return promo.target === 'SHIPPING' && promo.type === 'Free Shipping';
  };

  Handlebars.registerHelper('hasFreeShippingPromo', function(promotions, options) {
    var found = _.find (promotions, function(promo) {
      if (freeShipping(promo)){
        return promo;
      }
    });
  ...
  Handlebars.registerHelper('unlessFreeShippingPromo', function(promo, options) {
     if (!freeShipping(promo)){
      return options.fn(this);
    }else {
      return options.inverse(this);
  ...
  Handlebars.registerHelper('ifAllowedShippingMethod', function(method, options) {
     if (_.indexOf(['G', '2', '0'], method) !== -1) {
      return options.fn(this);
    }else {
      return options.inverse(this);
    }
  ...
  /*
   * Returns the number of items the user has in the bag
   */
  Handlebars.registerHelper('hasBagItems', function(options) {
    var itemType = options.hash.itemType || 'merged',
        data = { itemsCount: 0 };

    _.each(this.items, function(item) {
      var renderItem = (itemType == 'all')
                    || ((itemType == 'merged') && !item.mergeRequiredFlag)
                    || ((itemType == 'unmerged') && item.mergeRequiredFlag);
  ...
  /*
   * Executes the `each` block only for products that are in the bag (merge not required)
   */
  Handlebars.registerHelper('eachBagItem', function(items, options) {
    var filteredItems = [],
        itemType = options.hash.itemType || 'merged',
        index = 0,
        buffer = '',
    ...
    _.each(filteredItems, function(item) {
      data.index = index;
      data.first = (index === 0);
      data.last  = (index === (filteredItems.length-1));
  ...
  Handlebars.registerHelper('eachBagBopsItem', function(items, options) {
    var index = 0,
        buffer = '',
        data = _.extend({}, options.data);
    ...
    _.each(items, function(item) {
      ...
      data.collection  = options.hash.collection || '';
      buffer += options.fn(item, { data: data });
      index++;
    });
  ...
  Handlebars.registerHelper('ifBagBopsUnavailableStores', function(bopsAvailability, options) {
    var isBops, isPickup;

    var stores = _.filter(bopsAvailability.unavailable, function(item) {
      isBops = _.some(bopsAvailability.bops, function(bopsItem) {
        return bopsItem.store.locationNumber == item.store.locationNumber;
      })

      isPickup = _.some(bopsAvailability.pickup, function(pickupItem) {
        return pickupItem.store.locationNumber == item.store.locationNumber;
      })

      return !isBops && !isPickup;
    });

    if (stores.length) {
      return options.fn(this, { data: { stores: stores } });
    }

    return options.inverse(this);
  });

  Handlebars.registerHelper('ifSegmentCookie', function(SegmentID,options) {
    if(mvCookie.get('SEGMENT')){
      var segmentCookieValue = JSON.parse(mvCookie.get('SEGMENT'));
      if (segmentCookieValue.EXPERIMENT.indexOf(parseInt(SegmentID)) > -1 ){
        return options.fn(this);
      }
    }
    return options.inverse(this);
  ...
  Handlebars.registerHelper('promoCodeApplied', function(options) {
    var promoCodeOne = mvCookie.get('PromoCodeOne', 'SMISCGCs');
  	if (!_.isUndefined(promoCodeOne) && !_.isNull(promoCodeOne) && !_.isEmpty(promoCodeOne)){
      	return options.fn(this);
  	}
  	return options.inverse(this);
  });

  Handlebars.registerHelper('getPromoCodeValue', function(options) {
    var promoCodeOne = mvCookie.get('PromoCodeOne', 'SMISCGCs');

  	if (!_.isUndefined(promoCodeOne) && !_.isNull(promoCodeOne)){
      	return promoCodeOne;
  	}
  	return options.inverse(this);
  });

  Handlebars.registerHelper('checkStoreHour', function(storehour) {
    if (storehour === 'CLOSED'){
      return '<span class="storeClosed">CLOSED</span>'
    }
    return storehour;
  });

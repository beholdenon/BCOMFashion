...
var CHARLOTTE = {
	...
	cm: {
		category: "fall16_CharlotteTilbury"
	},
	...
	updateShop: function( data ) {
		...
		SERVICES.product.upcGet( function(res){
			...
			if ( res === "error" ) {
				...
				$('#signature-looks #products').css('min-height', $('#signature-looks #products').height() / $(window).width() *100 + 'vw' );
				$('.pagination .current').text(1);
				$('.pagination .total').text( Math.ceil( products.length/itemsPerPage ) );	
			}
...
$(document).ready( function () {
	...
	$(".essentials-video").each(function() {
		var tar = $(this),
			id = tar.attr('data-id');
	...
	$("#essential-arrows .left-arrow, #essential-arrows .right-arrow").on("click", function(ev) {
		ev.stopPropagation();
		if ( !$("#essential-arrows .left-arrow, #essential-arrows .right-arrow").hasClass('locked') ){
			$("#essential-arrows .left-arrow, #essential-arrows .right-arrow").addClass('locked');
			...
			$("#essential-arrows .arrow").animate({"opacity": 0}, 500).promise().then( function() {
				...
				$('#best-sellers [data-slide='+curPage+'] video')[0].play();
				...
				$('#best-sellers [data-slide='+curPage+'], #best-sellers [data-slide='+prevPage+'] ').animate({"left": "+=" + $(window).width() *direction }, 1000).promise().then(function() {
					...
					$('#best-sellers [data-slide='+prevPage+'] video')[0].pause();
					$('#best-sellers [data-slide='+prevPage+'] video')[0].currentTime = 0;
					$("#essential-arrows .arrow").animate({"opacity": 1}, 500).promise().then( function(){
						$("#essential-arrows .left-arrow, #essential-arrows .right-arrow").removeClass('locked');
					});
				});
	...
	$("[data-element]").on("click", function () {
		...
		if ( $(this)[0].hasAttribute('data-attribute') ) {
			...
			CHARLOTTE.coremetrics('Element', CHARLOTTE.cm.category, $(this).attr("data-element"), attrVal );
		} else {
			CHARLOTTE.coremetrics('Element', CHARLOTTE.cm.category, $(this).attr("data-element") );
		}
	...
	$("video").on("play", function() {
		...
		CHARLOTTE.coremetrics('Element', CHARLOTTE.cm.category, $(this).attr("data-video"), attrVal );
	});
	...
	$("#products").on('click', 'li.product', function() {
		...
		var attrNum = 29,
			...
			pName = removeDiacritics( targ.find('.name').text().replace('Charlotte Tilbury','').replace(/\&|\+|\,/g, '').replace(/\s+/g, '-') );
		...
		CHARLOTTE.coremetrics('Element', CHARLOTTE.cm.category, ('signature-looks_'+pName).slice(0, 50), attrVal );
	});

	// ARROW FUNCTIONALITY

	$("#products .left-arrow").on("click", function () {
		CHARLOTTE.currentPage --;
		if (CHARLOTTE.currentPage < 0) CHARLOTTE.currentPage = CHARLOTTE.markup.length-1;
		...
		$('.pagination .current').text(CHARLOTTE.currentPage+1);			
	});

	$("#products .right-arrow").on("click", function () {
		CHARLOTTE.currentPage ++;
		if (CHARLOTTE.currentPage >= CHARLOTTE.markup.length) CHARLOTTE.currentPage = 0;
	...
	sellerSwipe.on('swipe', function(ev) {
		...
		if ( ev.direction === 2 ) {
			$("#essential-arrows .right-arrow").trigger('click');
		} else if ( ev.direction === 4) {
			$("#essential-arrows .left-arrow").trigger('click');
		}
	});

	var productSwipe = new Hammer( document.getElementById('products') );
	productSwipe.on('swipe', function(ev) {
		//Hammer.js: left = 2, right = 4
		if ( ev.direction === 2 ) {
			$("#products .right-arrow").trigger('click');
		} else if ( ev.direction === 4) {
			$("#products .left-arrow").trigger('click');
		}
	});

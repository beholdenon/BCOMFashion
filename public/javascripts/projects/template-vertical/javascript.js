...
(function (window, $){
    ...
    var namespace = {
        ...
        environment: 'node', //local || node || legacy || astra 
    ...
    function floatingGrasphic () {
        if ($('#uniquePrefix_desktop_back_to_top').is(':in-viewport')){
            $('#uniquePrefix_desktop_floating_graphic').css('top','20%');
        } else {
    ...
    function initMobile(){
        ...
        } else if (namespace.environment == 'node'){
            loadMobileCSS('/fashion/styles/projects/template-vertical/mobile-style.css');
        } else {
    ...
    function socialShare(){
        ...
        try {
            if (namespace.environment == 'legacy'){
                ...
                var baseURLAssets = window.location.host;
            }
        } catch (e) {
            trace('Social share issue: ' + e);
        }
